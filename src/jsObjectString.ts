...
import { isPrimitive, Jsonable, JsonableObject } from './types.js';

export function jsObjectString(obj: JsonableObject): string {
  ...
  const seen: Jsonable[] = [];
  const shared: Jsonable[] = [];

  function scanForShared(obj: Jsonable): void {
    if ( isPrimitive(obj) ) {
      return;
    }
  ...
  function stringify(obj: Jsonable, disableShared?: boolean): string {
    indentCount++;
